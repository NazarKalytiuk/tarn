...
pub fn run_bench(
    ...
    let rt = tokio::runtime::Runtime::new()
        .map_err(|e| HiveError::Http(format!("Failed to create async runtime: {}", e)))?;
...
async fn run_bench_async(
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| HiveError::Http(format!("Failed to create HTTP client: {}", e)))?;
...
fn resolve_step(test_file: &TestFile, step_index: usize) -> Result<&Step, HiveError> {
    ...
    Err(HiveError::Config("No steps found in test file".into()))
}
...
"#;
    ...
    fn render_human_output() {
        ...
        assert!(output.contains("HIVE BENCH"));
