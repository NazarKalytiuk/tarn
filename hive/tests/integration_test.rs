...
fn start_demo_server(port: u16) -> Child {
    // The demo-server binary is built alongside hive
    let demo_bin = std::path::Path::new(env!("CARGO_BIN_EXE_hive"))
        .parent()
        .unwrap()
        .join("demo-server");
...
"#,
    ...
    hive().args(["validate", &good_file]).assert().success();

    let bad_file = write_test_file(&dir, "bad.hive.yaml", "not valid yaml: [");

    hive().args(["validate", &bad_file]).assert().code(2);
}
...
fn missing_file_exits_with_error() {
    hive()
        .args(["run", "/nonexistent/file.hive.yaml"])
        .assert()
        .code(2);
}
